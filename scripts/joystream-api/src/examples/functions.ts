...
export async function getParticipant(
  ...
  let memberId = -1;
  const isMemberRoot = (await api.query.members.memberIdsByRootAccountId(
    accountId
  )) as Vec<MemberId>;
  const isMemberCtrl = (await api.query.members.memberIdsByControllerAccountId(
    accountId
  )) as Vec<MemberId>;
  if (
...
export async function getParticipantAt(
  api: ApiPromise,
  accountId: AccountId,
  blockHash: Hash
): Promise<Participant> {
  let memberId = -1;
  const isMemberRoot = (await api.query.members.memberIdsByRootAccountId.at(
    blockHash,
    accountId
  )) as Vec<MemberId>;
  const isMemberCtrl =
    (await api.query.members.memberIdsByControllerAccountId.at(
      blockHash,
      accountId
    )) as Vec<MemberId>;
  if (
