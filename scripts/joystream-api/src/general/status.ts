...
async function main() {
  ...
  if (validators && validators.length > 0) {
    ...
    const totalValidatorBalances = validatorBalances.reduce(
      (total, value) => total.add(value.lockedBalance),
      new BN(0)
    );

    console.log(
      "Total Validator Locked Balances:",
      totalValidatorBalances.toString()
    );
  }
